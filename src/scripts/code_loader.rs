...
async fn split_file_into_chunks(
    ...
    log::info!("File Extension: {:?} Is Supported File {:}", is_supported_file.0, is_supported_file.1);
    ...
    if is_supported_file.1 == true && is_supported_file.0 != Language::Text {
        let splitter = CodeSplitter::new(
            get_language_from_file_extension(is_supported_file.0).context("Unsupported file extension")?,
            chunk_config,
